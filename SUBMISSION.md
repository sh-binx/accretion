...
- [x] **30초 플레이 영상** — `press/accretion-gameplay-30s.mp4` (1280×720 · 30fps · 7.4MB)
  - 담긴 장면: 타이틀 → **원시별**(코덱스 노출) → 점화·주계열성 → 초대질량 라이벌 → **조석 탈출** → **퀘이사 제트**
  - 제작 방법: Playwright **헤디드 + GPU**로 실제 60fps 렌더를 녹화(`headless:false`, SwiftShader 플래그 없이). 자동 조종은 리사주 궤적 — 텔레포트 없이 실제 이동이라 진짜 플레이처럼 보인다.
  - 주의: 헤드리스 + SwiftShader로 찍으면 게임 시간이 실시간의 0.3배로 흘러 못 쓴다(실측)
- [ ] 포털별 규격 크롭
