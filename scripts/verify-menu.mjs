...
try {
  ...
  ok('chip shows codex progress', /^3\/39$/.test(strip.codex), strip.codex)
  ...
  // 지평선(가로선)·시선축 격자선(세로선)이 '십자 아티팩트'로 보였다(오너 리포트).
  {
    ...
    // 그리드는 제거됐다(오너 4회 질의 → 5.19). 이제 검사의 뜻은 '어디서도 안 나온다'다.
    await new Promise(r=>setTimeout(r,1200))
    const play = await g.evaluate(() => window.__acc.gridState())
    ok('플레이: 격자 없음(제거됨)', play.vis===false && play.op<0.01, JSON.stringify(play))
    await g.close()
  }
