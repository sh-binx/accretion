...
try {
  ...
  ok('codex resets to 0/37', c0.count===0 && c0.total===37, JSON.stringify(c0))
  ...
  ok('grid shows all 37 entries', grid.total===37, `${grid.total}`)
  ok('8 discovered + 29 locked shown', grid.got===8 && grid.locked===29, `got=${grid.got} locked=${grid.locked}`)
