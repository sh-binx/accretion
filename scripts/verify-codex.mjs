...
try {
  ...
  ok('codex resets to 0/31', c0.count===0 && c0.total===31, JSON.stringify(c0))
  ...
  ok('grid shows all 31 entries', grid.total===31, `${grid.total}`)
  ok('8 discovered + 23 locked shown', grid.got===8 && grid.locked===23, `got=${grid.got} locked=${grid.locked}`)
