...
try {
  ...
  ok('codex resets to 0/33', c0.count===0 && c0.total===33, JSON.stringify(c0))
  ...
  ok('grid shows all 33 entries', grid.total===33, `${grid.total}`)
  ok('8 discovered + 25 locked shown', grid.got===8 && grid.locked===25, `got=${grid.got} locked=${grid.locked}`)
