...
try {
  ...
  ok('codex resets to 0/22', c0.count===0 && c0.total===22, JSON.stringify(c0))
  ...
  ok('AURORA still locked (<22)', c6.aurora===false)
  ...
  ok('grid shows all 22 entries', grid.total===22, `${grid.total}`)
  ok('8 discovered + 14 locked shown', grid.got===8 && grid.locked===14, `got=${grid.got} locked=${grid.locked}`)
