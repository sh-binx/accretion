...
try {
  ...
  ok('codex resets to 0/25', c0.count===0 && c0.total===25, JSON.stringify(c0))
  ...
  ok('grid shows all 25 entries', grid.total===25, `${grid.total}`)
  ok('8 discovered + 17 locked shown', grid.got===8 && grid.locked===17, `got=${grid.got} locked=${grid.locked}`)
