...
try {
  ...
  ok('codex resets to 0/23', c0.count===0 && c0.total===23, JSON.stringify(c0))
  ...
  ok('grid shows all 23 entries', grid.total===23, `${grid.total}`)
  ok('8 discovered + 15 locked shown', grid.got===8 && grid.locked===15, `got=${grid.got} locked=${grid.locked}`)
