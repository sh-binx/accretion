...
try {
  ...
  ok('codex resets to 0/38', c0.count===0 && c0.total===38, JSON.stringify(c0))
  ...
  ok('grid shows all 38 entries', grid.total===38, `${grid.total}`)
  ok('8 discovered + 30 locked shown', grid.got===8 && grid.locked===30, `got=${grid.got} locked=${grid.locked}`)
