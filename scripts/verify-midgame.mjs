...
try {
  ...
  const t = await page.evaluate(() => {
    ...
    for (const m of [20,50,100,200,5000]) { // 5티어 확장으로 400은 20과 너무 가까워 노이즈 → 간격 확대
      ...
      let tot=0, threat=0
      for (let r=0;r<45;r++){ A.clearObjs(); A.setMass(m); A.step(0.1)   // 표본 확대 — 20 vs 5000 차이가 몇 %p라 12회로는 동률이 나옴
  ...
  ok('damping still active at high mass', eff[5000]<0.16, `mass5000 eff=${eff[5000].toFixed(4)}`) // 에딩턴 완화(120→500) 후에도 한계는 유효
