...
try {
  ...
  const spawned = await page.evaluate(() => { const A=window.__acc; A.begin(); A.setMass(400); A.step(0.3); return A.nemesis() })
  ok('apex spawns once INTERMEDIATE black hole', spawned!==null, JSON.stringify(spawned))
  ok('apex is bigger than you (hunts)', spawned && spawned.mass>400 && spawned.vuln===false, spawned&&`mass=${spawned.mass} vuln=${spawned.vuln}`)

  // it hunts — distance closes when you hold still
  const hunt = await page.evaluate(() => { const A=window.__acc; const d0=A.nemesis().dist; for(let i=0;i<8;i++){A.setMass(400);A.step(0.2)} return { d0, d1:A.nemesis().dist } })
  ...
  const vuln = await page.evaluate(() => { const A=window.__acc; A.setMass(1400); A.step(0.1); return A.nemesis() })
  ...
  const climax = await page.evaluate(() => {
    ...
    for(let i=0;i<20;i++){A.setMass(1400);A.step(0.2)}
  ...
  const death = await page.evaluate(() => {
    const A=window.__acc; A.begin(); A.setMass(400); A.step(0.3)
    for(let i=0;i<60 && A.state.alive;i++){ A.setMass(400); A.step(0.2) } // it eventually catches a stationary hole
